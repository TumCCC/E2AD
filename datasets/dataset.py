...
def get_transform_tta(img_size, crop_size, train=True):
    if train:
        print('----------Transform----------------')
        transform = A.Compose([
            A.Resize(img_size, img_size),
...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.swa_epoch == 0 :
          ...
          return img_n.cuda(self.gpu), target
...
class AD_Dataset:
    ...
    def __init__(self,
        ...
        if transform:
            self.transform = get_transform_tta(img_size, crop_size, train)
        else:
    ...
    def get_data(self):
        ...
        if self.train:
            ...
            for sub_dir in os.listdir(os.path.join(self.data_dir, 'test')):
                #print(os.path.join(self.data_dir, 'test'))
    ...
    def get_dset(self):
        ...
        dset = BasicDataset(img_paths, targets, transform=self.transform, imagenet_norm=self.imagenet_norm, swa_epoch=self.swa_epoch, gpu=self.gpu)
        return dset
